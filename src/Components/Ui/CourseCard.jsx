...
export default function CourseCard({ course, renderStars }) {
  ...
  const handleLogin = () => {
    ...
    // Example: navigate('/login') or dispatch login action
  };

  const handleRegister = () => {
    setShowAuthWarning(false);
    // Redirect to register page or open register modal
    // Example: navigate('/register') or dispatch register action
  };
  ...
  return (
    ...
    if (!token) {
      ...
      }} className={`group bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 min-h-[480px] ${i18n.language === 'ar' ? 'text-right' : 'text-left'}`}>
        {/* Course Image */}
        <div className="relative overflow-hidden">
          <div className="w-full h-52 bg-gradient-to-br from-blue-50 to-purple-50 rounded-t-xl">
            <img 
