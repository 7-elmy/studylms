...
const months = Array.from({ length: 12 }, (_, i) => ({
  ...
  const semesters = [
    {
      value: 1,
      label: i18n.language === 'ar' ? 'الترم الأول' : 'First Semester'
    },
    {
      value: 2,
      label: i18n.language === 'ar' ? 'الترم الثاني' : 'Second Semester'
    }
  ];
  ...
  //   // Create form data based on package type
  //   const formData = new FormData();
  //   formData.append('package_id', packageId);

  //   // Check package type and add appropriate data
  //   const packageType = selectedPackage?.duration_label;
    
  //   if (packageType === t('packages.monthly') || packageType === 'شهري' || packageType === 'Monthly') {
  //     // For monthly packages, send month number
  //     formData.append('month', selectedMonth);
  //     console.log('Sending monthly subscription data:', {
  //       package_id: packageId,
  //       month: selectedMonth
  //     });
  //   } else if (packageType === t('packages.yearly') || packageType === 'سنوي' || packageType === 'Yearly' || 
  //              packageType === 'ترم' || packageType === 'Semester') {
  //     // For yearly/semester packages, send semester number
  //     formData.append('semester', selectedSemester);
  //     console.log('Sending semester subscription data:', {
  //       package_id: packageId,
  ...
  let requestData = { package_id: packageId };
  ...
  if (packageType === t('packages.monthly') || packageType === 'شهري' || packageType === 'Monthly') {
    ...
    packageType === t('packages.yearly') ||
    packageType === 'سنوي' ||
    packageType === 'Yearly' ||
    packageType === 'ترم' ||
    packageType === 'Term' ||
    packageType === 'Semester'
  ) {
    requestData.semester = selectedSemester;
    console.log('Sending semester subscription data:', requestData);
  }
console.log(requestData);
  ...
  await dispatch(
    apiRequest({
      ...
      url: `api/sub_scriptions/subscriptions`,
      method: "POST",
      data: requestData, // keep it as object
      headers: {
  ...
  // Check if package is monthly type
  const isMonthlyPackage = (duration) => {
    return duration === t('packages.monthly') || duration === 'شهري' || duration === 'Monthly';
  };

  // Check if package is semester/yearly type
  const isSemesterPackage = (duration) => {
    return duration === t('packages.yearly') || duration === 'سنوي' || duration === 'Yearly' || 
           duration === 'ترم' || duration === 'Semester' || duration === 'Term';
  };

  // Check if package is session type
  const isSessionPackage = (duration) => {
    return duration === t('packages.session') || duration === 'الحصة' || duration === 'Section';
  };
                  ...
                  <div className="text-center">
                    ...
                    <div className="bg-gray-50 rounded-xl p-6 mb-6">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {/* Monthly Package - Month Selection */}
                        {isMonthlyPackage(selectedPackage.duration_label) && (
                          <div className="bg-white rounded-lg p-4 shadow-sm">
                            <label className="block text-sm font-semibold text-gray-700 mb-3">
                              <svg className="w-5 h-5 inline mr-2 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                              </svg>
                              {i18n.language === 'ar' ? 'اختر عدد الأشهر:' : 'Select Number of Months:'}
                            </label>
                            <select
                              value={selectedMonth}
                              onChange={(e) => setSelectedMonth(parseInt(e.target.value))}
                              className="w-full p-3 border-2 border-gray-200 rounded-lg focus:border-emerald-500 focus:outline-none transition-colors text-gray-700 font-medium"
                            >
                              {months.map((month) => (
                        ...
                        {/* Semester Package - Semester Selection */}
                        {isSemesterPackage(selectedPackage.duration_label) && (
                          <div className="bg-white rounded-lg p-4 shadow-sm">
                            <label className="block text-sm font-semibold text-gray-700 mb-3">
                              ...
                              {i18n.language === 'ar' ? 'اختر الترم:' : 'Select Semester:'}
                            </label>
                            <select
                              value={selectedSemester}
                              onChange={(e) => setSelectedSemester(parseInt(e.target.value))}
                              className="w-full p-3 border-2 border-gray-200 rounded-lg focus:border-purple-500 focus:outline-none transition-colors text-gray-700 font-medium"
                            >
                      ...
                      <div className="mt-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                        <div className="flex items-center justify-center">
                          ...
                          <span className="text-blue-800 font-medium text-sm">
                            ...
                            {isSemesterPackage(selectedPackage.duration_label) && (
                              i18n.language === 'ar' 
                                ? `سيتم اشتراكك في ${semesters.find(s => s.value === selectedSemester)?.label}`
                                : `You will be subscribed to ${semesters.find(s => s.value === selectedSemester)?.label}`
                            )}
                            {isSessionPackage(selectedPackage.duration_label) && (
                              i18n.language === 'ar' 
                                ? 'يمكنك اختيار الدرس المناسب لك'
                                : 'You can choose your preferred lesson'
                            )}
                          </span>
                        </div>
                      </div>
                    ...
                    {/* <div className="mt-6 p-4 bg-gray-100 rounded-lg">
                      <h4 className="font-semibold text-gray-800 mb-2">
                        {i18n.language === 'ar' ? 'البيانات المرسلة:' : 'Data to be sent:'}
                      </h4>
                      <div className="bg-gray-800 text-green-400 p-3 rounded font-mono text-sm overflow-auto">
                        <pre>
{JSON.stringify({
  package_id: selectedPackage.id,
  ...(isMonthlyPackage(selectedPackage.duration_label) && { month: selectedMonth }),
  ...(isSemesterPackage(selectedPackage.duration_label) && { semester: selectedSemester })
}, null, 2)}
                        </pre>
                      </div>
                    </div> */}
...
//   const isSemesterPackage = (duration) => {
//     return duration === t('packages.yearly') || duration === 'سنوي' || duration === 'Yearly' || 
//            duration === 'ترم' || duration === 'Semester';
//   };
