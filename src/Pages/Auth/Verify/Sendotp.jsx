...
//             <strong>Demo:</strong> Enter "1234" as OTP to simulate successful verification.
//           </p>
//         </div>
//       </div>
//     </div>
...
const OTPVerify = () => {
  ...
  const handleVerify = async () => {
    if (!otp) {
      setOtpError(t('OTPVerify.errors.required'));
      return;
    }
    
    if (otp.length !== 4) {
      setOtpError(t('OTPVerify.errors.length'));
      return;
    }
  ...
  const handleResendOTP = async () => {
    ...
    try {
      ...
      setOtpError(t('OTPVerify.errors.resendFailed'));
    } finally {
      setIsResending(false);
    }
  };
      ...
      <div className="max-w-md w-full space-y-8">
        <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
          <div className="text-center mb-8">
            ...
            <h2 className="text-2xl font-bold text-gray-900 mb-2">{t('OTPVerify.title')}</h2>
            <p className="text-gray-600 text-sm">
          ...
          {resendSuccess && (
            <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg flex items-center">
              <CheckCircle className="h-5 w-5 text-green-500 mr-2" />
              <span className="text-sm text-green-700">{t('OTPVerify.resend.success')}</span>
            </div>
          )}
          ...
          <div className="space-y-6">
            ...
            >
              {isVerifying ? (
                <>
                  <RefreshCw className="animate-spin -ml-1 mr-2 h-4 w-4" />
                  {t('OTPVerify.verifyButton.verifying')}
                </>
              ) : (
                <>
                  <CheckCircle className="mx-1 h-4 w-4" />
                  {t('OTPVerify.verifyButton.default')}
                </>
              )}
            </button>
            ...
            <div className="text-center pt-4 border-t border-gray-100">
              <p className="text-sm text-gray-600 mb-3">
                {t('OTPVerify.resend.prompt')}
              </p>
              
              {countdown > 0 ? (
                <div className="flex items-center justify-center text-sm text-gray-500">
                  <Clock className="h-4 w-4 mr-1" />
                  {t('OTPVerify.resend.countdown', { countdown })}
                </div>
              ) : (
                <button
                  onClick={handleResendOTP}
                ...
                >
                  {isResending ? (
                    <>
                      ...
                      {t('OTPVerify.resend.button.sending')}
                    </>
                  ) : (
                    <>
                      <RefreshCw className="mx-2   h-3 w-3" />
                      {t('OTPVerify.resend.button.default')}
                    </>
                  )}
                </button>
              )}
