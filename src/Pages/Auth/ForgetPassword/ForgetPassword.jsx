...
// const ForgetPassword = ({onEmailSent}) => {
...
//         //    navigate("/send-otp")
//            navigate(`/otp-code?email=${encodeURIComponent(email)}`);
...
const ForgetPassword = ({ onEmailSent }) => {
  ...
  const handleSendOTP = async () => {
    if (!email) {
      setEmailError(t('forgetPassword.errors.emailRequired'));
      return;
    }
    
    if (!isValidEmail(email)) {
      setEmailError(t('forgetPassword.errors.invalidEmail'));
      return;
    }
    ...
    try {
      ...
      if (response.payload?.success) {
        ...
        setEmailError(response.payload?.message || t('forgetPassword.errors.sendFailed'));
      }
    } catch (error) {
      console.error('Send OTP Error:', error);
      setEmailError(error.response?.data?.message || t('forgetPassword.errors.sendFailed'));
    } finally {
      setIsSending(false);
    }
  };
        ...
        <div className="max-w-md w-full space-y-8">
          <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
            <div className="text-center">
              ...
              <h2 className="text-2xl font-bold text-gray-900 mb-2">{t('forgetPassword.success.title')}</h2>
              <p className="text-gray-600 text-sm mb-4">
                {t('forgetPassword.success.subtitle')}
              </p>
              <div className="flex items-center justify-center mb-6 p-2 bg-gray-50 rounded-lg">
                <Mail className="h-4 w-4 text-gray-400 mr-2" />
                <span className="text-sm font-medium text-gray-700">{email}</span>
              ...
              <div className="space-y-4">
                <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                  <p className="text-sm text-green-700">
                    {t('forgetPassword.success.instructions')}
                  </p>
                </div>
                ...
                <button
                  ...
                  className="w-full flex justify-center py-3 px-4 border border-gray-300 rounded-lg shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-500 transition-colors"
                >
                  {t('forgetPassword.success.tryAgain')}
                </button>
              </div>
            </div>
            ...
            <div className="mt-6 text-center">
              <p className="text-xs text-gray-500">
                {t('forgetPassword.success.notReceived')}
              </p>
            </div>
          </div>
          ...
          <div className="text-center">
            <p className="text-sm text-gray-600">
              {t('forgetPassword.success.expiry')}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-md w-full space-y-8">
        <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
          <div className="text-center mb-8">
            ...
            <h2 className="text-2xl font-bold text-gray-900 mb-2">{t('forgetPassword.title')}</h2>
            <p className="text-gray-600 text-sm">
          ...
          <div className="space-y-6">
            ...
            >
              {isSending ? (
                <>
                  <RefreshCw className="animate-spin -ml-1 mr-2 h-4 w-4" />
                  {t('forgetPassword.sendingButton')}
                </>
              ) : (
                <>
                  <Send className="-ml-1 mr-2 h-4 w-4" />
              ...
              <div className="flex items-start">
                ...
                <div>
                  <h4 className="text-sm font-medium text-blue-800 mb-1">
                    {t('forgetPassword.securityNotice.title')}
                  </h4>
                  <p className="text-xs text-blue-600">
                    {t('forgetPassword.securityNotice.description')}
                  </p>
                </div>
              </div>
            </div>
